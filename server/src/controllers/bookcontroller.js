...
const getBookByQueryParam = async (req, res) => {
    try {
        ...
        const books = await bookModel.find({isDeleted:false}).select({ title: 1, excerpt: 1, userId: 1, category: 1, releasedAt: 1, reviews: 1,ISBN:1 }).sort({ title: 1 });
...
const updateBook = async (req, res) => {
    try {
        ...
        let updatedBook = await bookModel.findOneAndUpdate({  isDeleted:false }, {$set : requestbody },{new:true})
...
const deleteById = async (req, res) => {
    try {
        ...
        let book = await bookModel.findOne({ title, isDeleted: false });
        if (!book) {
            return res.status(404).send({ status: false, message: "book does not exist" })
        }
        ...
        await bookModel.findOneAndUpdate({ title }, { isDeleted: true, deletedAt:moment().format("YYYY-MM-DD , hh:mm:ss ") }, { new: true });
